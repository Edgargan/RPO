...
def get_time_str():
    return datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")   # "%y%m%d-%H%M%S"
...
def generate_tag(seed, params):
    if 'tag' in params.keys():
        return
    game = params['game']
    params.setdefault('run', seed)
    run = params['run']
    del params['game']
    del params['run']
    str = ['%s_%s' % (k, v if is_plain_type(v) else v.__name__) for k, v in sorted(params.items())]
    # tag = '%s-%s-seed-%d' % (game, '-'.join(str), seed)
    tag = '%s--%s_seed_%d' % (game, '_'.join(str), seed)
    params['tag'] = tag
    params['game'] = game
    params['run'] = run


def generate_tag1(seed, tag, params):
    # if 'tag' in params.keys():
    game = params['game']
    # tag = params['tag']
    params.setdefault('run', seed)
